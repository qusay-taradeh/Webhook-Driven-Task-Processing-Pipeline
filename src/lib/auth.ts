...
import { createRefreshToken, NewRefreshToken } from './db/queries/refreshTokens.js';
...
export function validateJWT(tokenString: string, secret: string): string {
    ...
    if (typeof isVerified === "string") throw new Error("Invalid token: could not decode payload");

    if (!isVerified.sub) throw new Error("Invalid token: sub (userID) is missing");
...
export async function makeRefreshToken(userID: string) {
    ...
    const refreshToken: NewRefreshToken = {userId: userID, token: token, expiresAt: expiresAt60Days};
